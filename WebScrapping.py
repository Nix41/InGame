        ...
        if already.count() == 0:
            ...
            try: 
                ...
                if here.count() == 0:
                    ...
                    sess.commit()
                    ('    El juego ha sido descargado Exitosamente')
                    found.append(g)
            except TimeoutException:
                not_found += (g + '\n') 
                make_lists(g_list, found , 'lists/not_found_games.txt', not_found)
                ('    La pagina se demoro demasiado on ', g)
            except NoSuchElementException:
                not_found += (g + '\n')
                make_lists(g_list, found , 'lists/not_found_games.txt', not_found)
                ('    No se ha podido encontrar el juego', g ,' en la Pagina') 
        else:
            ('    Ya has hecho esta busqueda: ' + g)
    print('Done Download')
    driver.quit()
    print(found)
...
def build_serie(name, year, pais, sinopsis, generos, directors, reparto, image, score, topics):
    ...
    if already.count() == 0:
        ...
        with urllib.request.urlopen(image) as response, open(series_dir + str(serie.id) + slash + 'cover'+ str(datetime.now()) +'.jpeg', 'wb') as out_file:
            data = response.read()
            out_file.write(data)
        ('    La serie ha sido descargada exitosamente')
    else:
        ('    La serie ' + name + ' ya exitia')
...
def build_movie(name, year, pais, sinopsis, generos, directors, reparto, image, score, topics):
    ...
    if already.count() == 0:
        ...
        with urllib.request.urlopen(image) as response, open(movies_dir+ '/' + str(movie.id) + slash + 'cover'+ str(datetime.now()) +'.jpeg', 'wb') as out_file:
            data = response.read()
            out_file.write(data)
        ('    La pelicula ha sido descargada exitosamente')
    else: 
